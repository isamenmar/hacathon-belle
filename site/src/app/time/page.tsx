  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-gray-900 mb-3">Ranking de Atividades (2025+)</h3>
              ...
              {teamPerformers2026.map((p, i) => (
                <tr key={p.nome} className={`border-b border-gray-100 hover:bg-gray-50 ${p.nome.includes("Mia") ? "bg-[#FFF5F3]" : ""}`}>
                  ...
                  <td className="py-2 px-3 text-gray-700 font-medium">
                    {p.nome}
                    {p.nome.includes("Mia") && <span className="ml-1.5 inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-medium bg-[#F06B5D]/10 text-[#F06B5D]">IA</span>}
                    {p.nome.includes("Automação") && <span className="ml-1.5 inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-medium bg-yellow-100 text-yellow-700">BOT</span>}
                  </td>
                  <td className="text-right py-2 px-3 text-gray-900 font-medium">{formatNumber(p.total)}</td>
                  ...
                  <td className={`text-right py-2 px-3 ${p.noShows > 500 ? "text-red-500" : "text-gray-400"}`}>{p.noShows}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        <h3 className="text-sm font-semibold text-gray-900 mb-3">Top Vendedores por Valor (2024+)</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="text-gray-500 border-b border-gray-200">
            ...
            <tbody>
              {topSellers2026.filter(s => s.valorTotal > 0).sort((a, b) => b.valorTotal - a.valorTotal).slice(0, 12).map((s, i) => (
                <tr key={s.nome} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-2 px-3 text-gray-400">{i + 1}</td>
                  <td className="py-2 px-3 text-gray-700 font-medium">{s.nome}</td>
                  <td className="text-right py-2 px-3 text-gray-500">{s.dealsWon}</td>
